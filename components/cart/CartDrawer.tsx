...
export default function CartDrawer() {
    ...
    return (
        <>
            ...
            <div 
                id="CartPopup"
                className={`fixed top-0 right-0 bottom-0 w-full sm:max-w-[420px] bg-white z-[101] shadow-2xl flex flex-col transition-transform duration-300 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] ${
                ...
                <div className="p-5 md:p-6 bg-[#fafafa] border-b border-zinc-100">
                    <div className="text-center mb-4 text-sm text-[#444]">
                        {subtotal < freeShippingThreshold ? (
                            <p>Add <span className="font-bold">${(freeShippingThreshold - subtotal).toFixed(2)}</span> more to unlock <span className="font-bold">FREE SHIPPING</span>!</p>
                        ) : (
                            <p className="font-bold text-green-600">You've unlocked FREE SHIPPING!</p>
                        )}
                    </div>
                    ...
                    <div className="relative h-[6px] w-full bg-[#e0e0e0] rounded-full my-6">
                        ...
                        <div className="absolute top-1/2 left-0 right-0 -translate-y-1/2 flex justify-between px-[10%] pointer-events-none">
                            <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                                progress >= 50 ? 'bg-black border-black text-white' : 'bg-white border-[#e0e0e0] text-zinc-400'
                            }`}>
                            ...
                            <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                                progress >= 100 ? 'bg-black border-black text-white' : 'bg-white border-[#e0e0e0] text-zinc-400'
                            }`}>
                                <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div className="flex-grow overflow-y-auto px-6 scrollbar-hide">
                    {items.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-zinc-400 space-y-4 py-12">
                            ...
                            <h3 className="text-lg font-medium text-zinc-600">Your cart is empty</h3>
                            <button 
                                onClick={() => setIsDrawerOpen(false)}
                ...
                {items.length > 0 && (
                    <div className="p-6 border-t border-zinc-100 bg-white space-y-4 shadow-[0_-4px_10px_rgba(0,0,0,0.03)]">
                        <div className="flex justify-between items-center text-lg font-bold">
                            <span className="uppercase tracking-widest text-sm text-zinc-500">Subtotal</span>
                        ...
                        <div className="pt-2">
                            <Link 
                                href="/checkout"
                                onClick={() => setIsDrawerOpen(false)}
                                className="flex items-center justify-center w-full py-4 bg-black text-white text-sm font-bold uppercase tracking-[0.15em] transition-all hover:bg-[#222] active:scale-[0.98] rounded-sm"
                            >
