...
export default function BestSellerSection({ products }: { products: any[] }) {
    ...
    // Show all best seller products
    const displayProducts = products;

    const checkScroll = () => {
        if (scrollRef.current) {
            const { scrollLeft, scrollWidth, clientWidth } = scrollRef.current;
            setCanScrollLeft(scrollLeft > 5); // 5px threshold for better UX
            setCanScrollRight(scrollLeft < scrollWidth - clientWidth - 5);
        }
    };

    useEffect(() => {
        const timer = setTimeout(checkScroll, 100); // Small delay to let content render
