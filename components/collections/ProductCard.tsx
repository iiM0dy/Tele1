...
export default function ProductCard({ product, hideInfo, index, layout = 'medium', sizes }: ProductCardProps) {
    ...
    return (
        <div className={`flex flex-col group h-full bg-white rounded-2xl border border-zinc-100 shadow-sm transition-all duration-300 overflow-hidden ${isOutOfStock ? 'opacity-75 grayscale-[0.5]' : 'hover:shadow-xl hover:-translate-y-1'}`}>
            {/* Figure / Media Area */}
            <div className="relative w-full aspect-square overflow-hidden bg-white mx-auto">
                ...
                {!isOutOfStock && (
                    ...
                    >
                        <HiOutlineShoppingBag className="w-4 h-4 md:w-5 md:h-5" />
                    </button>
                )}
            </div>
