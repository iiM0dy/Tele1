...
export default function CartPage() {
    ...
    if (items.length === 0) {
        return (
            <div className="min-h-[60vh] bg-background flex flex-col items-center justify-center px-4">
                ...
                <h1 className="text-3xl font-medium mb-4 uppercase tracking-widest">{t('cart.emptyCart')}</h1>
                <Link 
    ...
    return (
        <div className="w-full min-h-screen bg-background px-4 md:px-[48px] pt-16 pb-12 md:pb-20 text-[rgb(18,18,18)]">
            <div className="max-w-[980px] mx-auto">
                <h1 className="text-center text-[31px] font-bold tracking-[0.2em] uppercase mb-16 leading-tight">{t('cart.yourCart')}</h1>

                {/* Desktop Table Header */}
                <div className="hidden md:grid grid-cols-[2fr_1fr_1fr] border-b border-[#e1e1e1] pb-4 text-[13px] uppercase tracking-widest font-bold text-[rgb(18,18,18)]">
                ...
                <div className="space-y-0 mb-8 md:mb-12">
                    {items.map((item) => (
                        <div key={item.id} className="flex flex-row md:grid md:grid-cols-[2fr_1fr_1fr] items-start md:items-center gap-4 md:gap-6 border-b border-[#f5f5f5] py-6 md:py-0 h-auto md:h-[153px]">
                            ...
                            <div className="flex gap-4 md:gap-6 items-start md:items-center flex-1 md:flex-none md:col-span-1 h-full">
                                ...
                                <div className="flex flex-col justify-center gap-1 md:gap-2">
                                    <Link href={`/products/${item.id}`} className="hover:underline underline-offset-4">
                                        <h3 className="text-[13px] font-bold uppercase tracking-[0.2em] text-[rgb(18,18,18)]">
                                            {item.name}
                                        </h3>
                                    </Link>
                                    ...
                                    <div className="flex md:hidden items-center gap-4 mt-2">
                                        ...
                                        <button onClick={() => removeItem(item.id)} className="text-[12px] uppercase tracking-widest font-medium underline underline-offset-4">{t('cart.remove')}</button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="hidden md:flex flex-col items-center justify-center gap-2 h-full">
                                <div className="flex items-center border border-[#e1e1e1] h-10">
                                    ...
                                    <input 
                                        ...
                                        className="w-12 text-center text-[13px] font-bold text-[rgb(18,18,18)] focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                    />
                                    <button 
                                ...
                                <button 
                                    ...
                                    className="text-[12px] uppercase tracking-widest font-bold text-[rgb(18,18,18)] underline underline-offset-4 hover:text-primary transition-colors"
                                >
                                    {t('cart.remove')}
                                </button>
                            </div>
                            ...
                            <div className="hidden md:flex text-right font-bold text-[16px] text-[rgb(18,18,18)] items-center justify-end h-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-8 border-t border-[#e1e1e1]">
                    ...
                    <div className="space-y-4">
                        <button className="text-[14px] uppercase tracking-widest font-bold text-[rgb(18,18,18)] flex items-center gap-2">
                            <span className="text-lg leading-none mt-[-2px]">•</span> {t('cart.addOrderNote')}
                        </button>
                        <textarea 
                            value={orderNote}
                            onChange={(e) => setOrderNote(e.target.value)}
                            placeholder={t('cart.orderNotePlaceholder') }
                            className="w-full h-32 border border-[#e1e1e1] rounded-md p-4 text-[0.9rem] focus:outline-none focus:ring-1 focus:ring-primary transition-all text-[rgb(18,18,18)]"
                        />
                    </div>

                    {/* Summary & Checkout */}
                    <div className="flex flex-col items-end gap-6">
                        <div className="text-[16px] font-bold text-[rgb(18,18,18)]">
                            {t('cart.total')}: ${subtotal.toFixed(2)} USD
                        </div>
                        <Link 
                            href="/checkout"
                            className="w-[148px] h-[60px] bg-primary text-white flex items-center justify-center text-[13px] font-bold uppercase tracking-[0.2em] hover:bg-primary/90 transition-all rounded-xl"
                        >
