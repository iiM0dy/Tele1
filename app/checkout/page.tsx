                ...
                <aside className={`lg:bg-[#f5f5f5] lg:border-l lg:border-[#e1e1e1] flex justify-center lg:justify-start order-1 lg:order-2 transition-colors duration-300 ${isSummaryOpen ? 'bg-[#f5f5f5]' : 'bg-white'}`}>
                    <div className={`w-full max-w-[404px] grid transition-all duration-300 ease-in-out lg:grid-rows-[1fr] lg:opacity-100 ${isSummaryOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                                <div className={`overflow-hidden px-4 md:px-8 lg:px-12 py-[30px] lg:py-16 lg:border-b-0 ${isSummaryOpen ? 'border-b border-[#e1e1e1]' : ''}`}>
                                    ...
                                    {/* Item List */}
                                    <div className="space-y-4 mb-8 px-2 pr-4">
                                        {items.map((item) => (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8 text-left">
                            ...
                            <div className="lg:hidden mt-8 border-t border-[#e1e1e1] pt-4">
                                        ...
                                        <div className="flex justify-between items-start">
                                            <div className="flex items-center gap-3">
                                                <div className="flex items-center">
                                                    {items.slice(0, 2).map((item, idx) => (
                                                        <div
                                                            key={item.id}
                                                            className={`relative w-10 h-10 bg-white border border-[#e1e1e1] rounded-[4px] shadow-sm overflow-hidden z-10`}
                                                            style={{ 
                                                                marginInlineStart: idx > 0 ? '-24px' : '0px',
                                                                zIndex: 10 - idx
                                                            }}
                                                        >
                                                            <Image
                                                    ...
                                                    {items.length > 2 && (
                                                        <div 
                                                            className="relative w-10 h-10 bg-[#f5f5f5] border border-[#e1e1e1] rounded-[4px] z-0 flex items-center justify-center text-[0.7rem] text-[#666] font-medium pt-1"
                                                            style={{ marginInlineStart: '-24px' }}
                                                        >
                                                            <span className={language === 'ar' ? 'pr-4' : 'pl-4'}>+{items.length - 2}</span>
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="flex flex-col min-w-0">
                                                    <span className="text-[1.1rem] font-medium text-[#1a1a1a] leading-tight truncate">{t('cart.total')}</span>
                                                    <span className="text-[0.85rem] text-[#707070] truncate">{totalItems} {t('items')}</span>
                                                </div>
                                            </div>
                                            <div className="flex flex-col items-end shrink-0 ml-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <svg width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="currentColor" className={`transition-transform duration-300 ${isFormSummaryOpen ? 'rotate-180' : ''}`}>
                                                        <path d="m2 5 5 5 5-5" strokeWidth="1.2" />
                                                    </svg>
                                                </div>
                                                {totalSavings > 0 && (
                                ...
                                <div className={`grid transition-all duration-300 ease-in-out ${isFormSummaryOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                                    <div className="overflow-hidden">
                                        <div className="space-y-4 pt-4 px-2 pr-4">
                                            {/* Item List */}
