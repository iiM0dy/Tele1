            ...
            <div className="relative bg-[#202126] w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-white/5">
                ...
                <div className="px-6 py-5 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
                    <div>
                        ...
                        <p className="text-[10px] text-white/60 font-black tracking-[0.2em] mt-1">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-hide">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 p-5 rounded-2xl bg-white/[0.02] border border-white/5">
                        <div className="space-y-2">
                            <p className="text-[10px] font-black tracking-[0.2em] text-white/60">{t('admin.currentStatus')}</p>
                        ...
                        <div className={`space-y-1 ${dir === 'rtl' ? 'text-left' : 'text-right'}`}>
                            <p className="text-[10px] font-black tracking-[0.2em] text-white/60">{t('admin.totalAmount')}</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            <h4 className="text-[10px] font-black tracking-[0.2em] text-white flex items-center gap-2">
                            ...
                            <div className={`space-y-1 ${dir === 'rtl' ? 'mr-10' : 'ml-10'}`}>
                                <p className="text-xs font-black text-white tracking-tight">{order.Name}</p>
                                <p className="text-[11px] font-black text-white/60 tracking-widest" dir="ltr">{order.phone}</p>
                            </div>
                        </div>
                        <div className="space-y-4">
                            <h4 className="text-[10px] font-black tracking-[0.2em] text-white flex items-center gap-2">
                            ...
                            <div className={`space-y-1 ${dir === 'rtl' ? 'mr-10' : 'ml-10'}`}>
                                <p className="text-[11px] font-black text-white/60 tracking-widest leading-relaxed">
                        ...
                        <div className="border border-white/5 rounded-3xl overflow-hidden bg-white/[0.01]">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-white/[0.02] border-b border-white/5">
                                    <tr>
                                        <th className={`p-4 text-[10px] font-black tracking-[0.2em] text-white/60 ${dir === 'rtl' ? 'text-right' : 'text-left'}`}>{t('admin.product')}</th>
                                        <th className="p-4 text-[10px] font-black tracking-[0.2em] text-white/60 text-center">{t('admin.qty')}</th>
                                        <th className={`p-4 text-[10px] font-black tracking-[0.2em] text-white/60 ${dir === 'rtl' ? 'text-left' : 'text-right'}`}>{t('admin.price')}</th>
                                        <th className={`p-4 text-[10px] font-black tracking-[0.2em] text-white/60 ${dir === 'rtl' ? 'text-left' : 'text-right'}`}>{t('admin.total')}</th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr key={idx} className="hover:bg-white/[0.02] transition-all group">
                                            <td className="p-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-[10px] font-black tracking-widest text-white line-clamp-1">
                                                        {item.product?.name || t('admin.deletedProduct')}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className="p-4 text-center text-[10px] font-black tracking-widest text-white/60">
                                                {item.quantity}
                                            </td>
                                            <td className={`p-4 text-[10px] font-black tracking-widest text-white/60 ${dir === 'rtl' ? 'text-left' : 'text-right'}`}>
                                                ${Number(item.price).toFixed(2)}
                                            </td>
                                            <td className={`p-4 text-[10px] font-black tracking-widest text-white ${dir === 'rtl' ? 'text-left' : 'text-right'}`}>
                ...
                <div className={`px-6 py-5 bg-white/[0.01] border-t border-white/5 flex items-center gap-3 ${dir === 'rtl' ? 'justify-start' : 'justify-end'}`}>
                    {canDelete && onDelete && (
                        <button
                            ...
                            onClick={onDelete}
                            disabled={isDeleting}
                            className="h-11 px-5 rounded-xl font-black text-[10px] tracking-widest border border-red-500/20 text-red-400 hover:bg-red-500/10 transition-all disabled:opacity-50 flex items-center gap-2"
                    ...
                    <button
                        onClick={onClose}
                        className="bg-accent hover:bg-accent/90 text-white h-11 px-8 rounded-xl font-black text-[10px] tracking-widest transition-all active:scale-95"
                        aria-label={t('admin.close')}
                    >
                        {t('admin.close')}
                    </button>
